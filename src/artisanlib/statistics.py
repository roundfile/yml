...
# AUC things
#TODO done - Leave as is.  get rid of From Event, make a blank From entry and then adjust save/load settings and correct the QComboBox index for the blank entry
#TODO done - Seems to be.  investigate if aw.qmc.AUCbackground is set outside of here?  What happens when a new bkgd profile gets opened after this dialog is closed??
#TODO done-write tool tips for the Tab 1 statistics items
#TODO done- fix AUC to update in real time
#TODO done-rewrite the whole statistics tab1 handling, the saving of choices is redundant and strange

# canvas:statsSummary() things
#TODO need to find a common unicode character for truncated lines
#     the downarrowUTF character needs to be confirmed/explored on Linux
#     or just get rid of the whole thing

# Config Summary things
#TODO add more stats
#TODO adjust summary stats display using decimal places?
#TODO enable auto scroll in events buttons and alarms?
#TODO fixed? the vertical text sizing does not work great when changing fonts live in UI tab.  After clicking OK all is well.  Probably
#     it's the extra redraw()? It seems like the old font parameters are still active when the resizing is done.  Maybe it is a
#     processEvents() thing?  Very annoying because it works perfectly in the Statistics dialog.
#TODO fixed? Not sure how?? why is the stats box smaller, one character or so, when opening the profile first time?  Independent of auto axis.
#     Redraw does it. Start Artisan with a profile already showing.  Too small.  Load recent file (same) and the size
#     is okay.  Start Artisan with no profile.  Load recent and the size is okay.
#TODO QComboBox background color and text need to work for dark mode
#TODO done-is it necessary in deletesummarystat to disconnect signals?? No way to tell so keep to be safe
...
class StatisticsDlg(ArtisanDialog):
    def __init__(self, parent:'QWidget', aw:'ApplicationWindow', activeTab:int = 0) -> None:
        ...
        _log.info("Entry into StatisticsDlg")  #dave
...
#        self.helpdialog = None
        ...
        self.beginComboBox.currentIndexChanged.connect(self.AUCBeginChanged)  #dave
...
#        vgroupLayout.addWidget(statsmaxchrperlineGroupLayout)
        ...
        #TODO do the 'roast' entries need to use __dijstra_to_ascii()?  If so, I don't know how to call it from here.
        self.summarystats_types:List[str] = [ # presentation of these stats is done with corresponding entries in canvas:statsSummary())
                QApplication.translate('ComboBox','Blank Line'),                      # 0
                QApplication.translate('ComboBox','Title'),                           # 1
                QApplication.translate('ComboBox','Roast Date, Time'),                # 2
                QApplication.translate('ComboBox','Roast of the Day'),                # 3
                QApplication.translate('ComboBox','Ambient Temp, Hum, Pres'),         # 4
                QApplication.translate('ComboBox','Roaster, RPM'),                    # 5
                QApplication.translate('ComboBox','Bean'),                            # 6
                QApplication.translate('ComboBox','Screen Size'),                     # 7
                QApplication.translate('ComboBox','Density Green'),                   # 8
                QApplication.translate('ComboBox','Moisture Green'),                  # 9
                QApplication.translate('ComboBox','Batch Size'),                      # 10
                QApplication.translate('ComboBox','Density Roasted'),                 # 11
                QApplication.translate('ComboBox','Moisture Roasted'),                # 12
                QApplication.translate('ComboBox','Ground Color'),                    # 13
                QApplication.translate('ComboBox','Energy'),                          # 14
                QApplication.translate('ComboBox','CO2'),                             # 15
                QApplication.translate('ComboBox','AUC'),                             # 16
                QApplication.translate('ComboBox','Notes (Roast)'),                   # 17
                QApplication.translate('ComboBox','Cupping Score'),                   # 18
                QApplication.translate('ComboBox','Notes (Cupping)'),                 # 19
                QApplication.translate('ComboBox','Weight Green'),                    # 20
                QApplication.translate('ComboBox','Weight Roasted'),                  # 21
                QApplication.translate('ComboBox','Weight Loss'),                     # 22
                QApplication.translate('ComboBox','BBP Total Time'),                  # 23
                QApplication.translate('ComboBox','BBP Bottom Temp'),                 # 24
                QApplication.translate('ComboBox','BBP Summary'),                     # 25
                QApplication.translate('ComboBox','BBP Summary long'),                # 26
                QApplication.translate('ComboBox','BBP Summary compact'),             # 27
                                     ]
...
#TODO Removed the Help button to make room for a Default button
#        helpDialogButton = QDialogButtonBox()
...
#        statstablebuttonslayout.addSpacing(15)
 ...
 #       statstablebuttonslayout.addSpacing(15)
        ...
        statstablebuttonslayout.addStretch()
        #statstablebuttonslayout.addWidget(self.showalwaysCheckBox)
...
#        tab2buttonlayout.addWidget(helpDialogButton)
    ...
    def accept(self) -> None:
        ...
        #TODO #dave check as the statements above are removed
        assert self.aw.qmc.AUCbegin == self.beginComboBox.currentIndex()
        assert self.aw.qmc.AUCbase == self.baseedit.value()
        assert self.aw.qmc.AUCbaseFlag == self.baseFlag.isChecked()
        assert self.aw.qmc.AUCtarget == self.targetedit.value()
        assert self.aw.qmc.AUCtargetFlag == self.targetFlag.isChecked()
        assert self.aw.qmc.AUCguideFlag == self.guideFlag.isChecked()
        assert self.aw.qmc.AUClcdFlag == self.AUClcdFlag.isChecked()
    ...
    def moveItem(l:List[Any], source:int, target:int) -> None:
        _log.info("Inside moveItem")  #dave
    ...
    def sectionMoved(self, logicalIndex:int, _oldVisualIndex:int, newVisualIndex:int) -> None:
        ...
        _log.info("Inside sectionMoved")  #dave
...
#        self.closeHelp()
    ...
    def setstatsmaxchrperline(self, _:int) -> None:
        ...
        self.aw.qmc.redraw(recomputeAllDeltas=False)  #dave
...
#        self.summarystatstable.clearSelection() # this seems to work also for Ubuntu 16.04
        ...
        self.summarystatstable.setAutoScroll(True)  #dave False
        vheader = self.summarystatstable.verticalHeader()
        if vheader is not None:
            vheader.setSectionsMovable(True)
            vheader.setDragDropMode(QTableWidget.DragDropMode.InternalMove)
            vheader.setAutoScroll(False)  #dave False(False)
...
#            typeComboBox.setStyleSheet(f"background: {'#ffffff'}")  #dave temporary, need to allow for dark mode
#            #1 Empty Cell
#            emptyCell = QLabel('')
...
#                pass
        ...
        self.savetablesummarystats()  #dave
    ...
    def deletesummarystat(self, _:bool = False) -> None:
        ...
        _log.info("-- In deletesummarystat, bindex_pre=%s, bindex=%s, selected=%s",len(self.summarystatstypes)-1,bindex,selected)  #dave
        if bindex >= 0:
    ...
    def insertsummarystat(self, insert:bool = False) -> None:
        try:
            ...
            _log.exception("***** Failed in insertsummarystat")  #dave
            pass
...
#        stat_visibility:int = 1
        ...
        if bindex >= 0:
            ...
            if len(self.summarystatstypes) > 1:
                self.repaint()
            if not insert:
                _log.info('bindex= %s, len(self.summarystatstypes)= %s, self.summarystatstable.rowCount()= %s',bindex,len(self.summarystatstypes),self.summarystatstable.rowCount())  #dave
                _log.info('self.summarystatstable.item(self.summarystatstable.rowCount()-1, 0)= %s',self.summarystatstable.item(self.summarystatstable.rowCount()-1, 0))  #dave
    ...
    def updatetypes(self) -> None:
        try:
            _log.info("Inside updatetypes")  #dave
#            self.closeHelp()
#dave stop remembering
#            # save column widths
...
#    @pyqtSlot(int)
#    def changeShowalwaysflag(self, _:int) -> None:
#        self.aw.summarystatsshowalwaysflag = not self.aw.summarystatsshowalwaysflag
...
#        self.showalwaysCheckBox.setChecked(False)
        self.createSummarystatsTable()
        #self.savetablesummarystats(True)

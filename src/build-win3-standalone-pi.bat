...
rem ::
rem :: package the installation zip file
rem ::
...
rem     if ERRORLEVEL 1 (echo ** Failed in 7z zipping the setup files & exit /b 1)
rem )
rem 
rem ::
rem :: check that the packaged files are above an expected size
rem ::
rem set file=artisan-%ARTISAN_SPEC%-%ARTISAN_VERSION%.zip
rem set min_size=170000000
rem for %%A in (%file%) do set size=%%~zA
rem if %size% LSS %min_size% (
rem     echo *** Zip file is smaller than expected
rem     exit /b 1
rem ) else (
rem     echo **** Success: %file% is larger than minimum %min_size% bytes
rem )
